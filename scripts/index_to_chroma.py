...
Usage (called by GitHub Actions with repo-root-relative paths):
    python Gateway-testcases-csv/scripts/index_to_chroma.py \\
        Gateway-testcases-csv/adyen_direct_intergration/add_card.csv \\
        Gateway-testcases-csv/adyen_direct_intergration/wallet.csv

    # Reindex every CSV in the default gateway directory:
    python Gateway-testcases-csv/scripts/index_to_chroma.py --all
...
def main() -> None:
    ...
    parser.add_argument(
        "--reset-collection",
        action="store_true",
        help="Delete and recreate the Chroma collection before indexing (use when changing embedding model/dimension).",
    )
    args = parser.parse_args()

    if not args.files and not args.all:
        parser.error("Provide CSV file paths or pass --all to reindex everything.")
    ...
    if args.reset_collection:
        try:
            ...
            print(f"Deleted collection: {COLLECTION_NAME!r}")
        except Exception as e:
            print(
                f"[warn] Failed to delete collection {COLLECTION_NAME!r} (may not exist yet): {e}",
                file=sys.stderr,
            )
    ...
    csv_paths: list[Path] = []
    if args.all:
        csv_paths = sorted(args.gateway_dir.glob("*.csv"))
        if not csv_paths:
